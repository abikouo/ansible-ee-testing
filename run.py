...
def skip_target(path, targets) -> bool:
    
    if targets and path.stem not in targets:
        return True
    
    runme = path / PosixPath("runme.sh")
    if runme.exists():
        print("[SKIP] ", path.stem, " contains 'runme.sh' file.")
        return True
    ...
    if any(x.startswith("disabled") for x in attributes) or any(x.startswith("unsupported") for x in attributes):
    ...
    for x in attributes:
        ...
        if m:
            if int(m.group(1)) > 5:
                print("[SKIP] ", path.stem, " too long, estimated time = ", m.group(1), "min")
                return True
    return False


def run(eei_image, collection_path, vars_file, targets, use_stdout):
    variables = {}
    if vars_file:
    ...
    for target in path.iterdir():
        ...
        print("++++++++++++++++++++++++++++++++++++++++++++++++++\033[00m")
        if use_stdout:
            cmd = subprocess.Popen(" ".join(command), shell=True)
        else:
            cmd = subprocess.Popen(" ".join(command), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
...
def main() -> None:
    ...
    parser.add_argument("--use-stdout", action='store_true', help="Use stdout for the execution output")
